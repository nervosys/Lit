...
fn build_commands() -> Vec<CommandDef> {
    vec![
        ...
        cmd("mcp-serve", "MCP Tool Server", "server", "Start the Model Context Protocol (MCP) tool server for LLM agent integration. Exposes lit.* tools via JSON-RPC 2.0",
            ...
            vec![], vec!["init"], false, false),
        cmd("server serve", "Self-Hosted Server", "server", "Start the hardened, self-hostable server: named accounts, role-based authorization, expiring sessions, TLS, a system use notification, and an audit record for every authentication and authorization decision. Unlike `serve`, which is a loopback development server with an optional shared token, this is the command intended to be hosted. Refuses to serve plaintext on a non-loopback address unless --allow-plaintext is passed. See docs/SELF_HOSTING.md and docs/NIST_800-171.md",
            vec![
                param("bind", "string", "Address to bind; a routable address requires TLS", false, Some("127.0.0.1")),
                param("port", "integer", "Port to listen on", false, Some("3000")),
                param("tls-cert", "string", "PEM certificate chain", false, None),
                param("tls-key", "string", "PEM private key, unencrypted, owner-readable only", false, None),
                param("banner", "string", "System use notification shown before authentication (NIST SP 800-171r3 03.01.09)", false, None),
                param("users", "string", "Account store path", false, Some("~/.lit/server/users.json")),
                param("audit-log", "string", "Audit log path", false, Some("~/.lit/audit.log")),
                param("no-audit", "boolean", "Disable audit logging; recorded at startup when used", false, Some("false")),
                param("allow-plaintext", "boolean", "Serve plaintext on a routable address anyway", false, Some("false")),
                param("idle-timeout", "integer", "Seconds of inactivity before a session is terminated", false, Some("900")),
                param("max-lifetime", "integer", "Seconds after creation before a session is terminated", false, Some("28800")),
                param("max-attempts", "integer", "Consecutive failed logons before an account locks", false, Some("5")),
                param("lockout-secs", "integer", "Seconds an account stays locked; 0 means until an admin unlocks it", false, Some("900")),
            ],
            "ServeResponse",
            vec!["Starts a long-running HTTP/HTTPS server", "Writes audit records", "Updates last_login in the account store"],
            vec!["Repository must be initialized", "At least one account must exist"],
            vec![
                ex("Loopback", "lit server serve --bind 127.0.0.1 --port 3000 --banner /etc/lit/banner.txt",
                   serde_json::json!({"bind": "127.0.0.1", "port": 3000, "banner": "/etc/lit/banner.txt"})),
                ex("With TLS", "lit server serve --bind 0.0.0.0 --port 8443 --tls-cert /etc/lit/tls/fullchain.pem --tls-key /etc/lit/tls/privkey.pem",
                   serde_json::json!({"bind": "0.0.0.0", "port": 8443, "tls-cert": "/etc/lit/tls/fullchain.pem", "tls-key": "/etc/lit/tls/privkey.pem"})),
            ],
            vec![], vec!["init", "server user add"], false, false),
        cmd("server user", "Server Account Administration", "server", "Manage self-hosted server accounts: add, list, role, password, disable, enable, remove, unlock. Passwords are never taken as a command-line argument, because arguments are visible in the process table; use --password-file, --password-stdin, or LIT_SERVER_PASSWORD. Changes are picked up by a running server, but do not terminate sessions that account already holds",
            vec![
                param("action", "string", "add | list | role | password | disable | enable | remove | unlock", true, None),
                param("username", "string", "Account name", false, None),
                param("role", "string", "reader | contributor | maintainer | admin", false, Some("reader")),
                param("password-file", "string", "File containing the password", false, None),
                param("password-stdin", "boolean", "Read the password from stdin", false, Some("false")),
                param("users", "string", "Account store path", false, Some("~/.lit/server/users.json")),
            ],
            "ServerAdminResponse",
            vec!["Creates or modifies the account store", "Refuses to remove, demote, or disable the only enabled admin"],
            vec!["Password must be at least 15 characters and must not contain the username"],
            vec![
                ex("First administrator", "lit server user add alice --role admin --password-stdin",
                   serde_json::json!({"action": "add", "username": "alice", "role": "admin", "password-stdin": true})),
                ex("List accounts", "lit server user list", serde_json::json!({"action": "list"})),
                ex("Revoke", "lit server user remove dave", serde_json::json!({"action": "remove", "username": "dave"})),
            ],
            vec!["server serve"], vec![], false, false),
        // Git interop
