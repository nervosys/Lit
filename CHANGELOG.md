...
- `cargo test` now runs **420 tests** (plus 2 ignored), up from 382 at 1.0.0, and all 420 are distinct — the same suite previously reported 508 by running 94 unit tests in both targets. Two suites that had been disabled rather than fixed are running again
