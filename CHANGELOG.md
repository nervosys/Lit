...
- **The server serves the repository it was handed, not the one its working directory happens to point at** — `route_request` resolved the repository two ways: twelve places honoured the `repo_root` passed to it, while every route delegating to `commands::*` called `find_repo_root()` on the process's working directory. Through `lit server serve` those always agreed, so nothing was visibly broken; through the `bind()` entry point they did not, and half the routes would act on each. Fourteen commands gained an `execute_at(repo_root, …)` and `execute(…)` now delegates to it, following the `serve::execute_at` pattern already in the tree. This is additive — no existing signature changed, so nothing breaks for library consumers. `config::execute` is unchanged, having never read the repository. The invariant is held by an integration test that serves a scratch repository deliberately elsewhere from the test process's working directory, so a regression fails rather than going unnoticed
